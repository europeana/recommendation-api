...
public class RecommendController {
    ...
    /**
     * Given an entity type and id number, this returns a json response containing basic data about similar records
    ...
    public ResponseEntity<SearchApiResponse> acceptEntity(
        ...
        validateRecordIds(ids);
        recommendService.submitUserSignals(ids, RecommendationConstants.USER_SIGNAL_ACCEPT, userId, wskey, authToken);
        return recommendEntity(type, id, ids.length, wskey, authToken);
    }
    ...
    /**
     * Check if we have a token and/or API key. If not we throw an error, otherwise we'll return either
     * 1) userId - from the token provided (token must be present for this and getUserFromToken must be true)
     * 2) apikey - from the token provided or wskey passed
     * Note that we don't actually do API key validation ourselves. It's done implicitely when reusing API key
     */
